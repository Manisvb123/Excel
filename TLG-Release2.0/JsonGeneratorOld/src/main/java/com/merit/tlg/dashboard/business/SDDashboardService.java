 ...
 * different charts required by Structural Dynamic
 * 
 */
public class SDDashboardService extends BaseDashboardService {
	final static Logger logger = Logger.getLogger(SDDashboardService.class);
	...
	public InsightData generateInsightData(String orgId, String toolId, String projectId) {
		...
		insData.setTitle("SD Insight");
		insData.setDescription("SD insights shows how good the strategy is ");
		...
		insData = insightDashboardService.generateInsightData(insData, orgId, toolId, projectId,
				CommonConstants.SD_INSIGHT_DASHBOARD_ID);
		logger.debug("generateInsightData: Insight data from SD " + insData);
	...
	public IndexData generateIndexData(String orgId, String toolId, String projectId) {
		...
		indexData.setTitle("SD Summary ");
		indexData.setType("Guage");
		indexData.setDescription("SD Summary shows the primary dimension scores (as percent scores)");
		...
		writeToFileAndUpdateDashboard(indexData, orgId, toolId, projectId, CommonConstants.SD_INDEX_DASHBOARD_ID);
		logger.debug("generateIndexData: Completed Writing the index json file for SD in the specified folder");
	 ...
	 * Chart for SD for a particular question .
	 * 
	 * @param orgId: The organization Id.
	 * @param toolId: The tool Id.
	 * @param projectId: The project Id.
	...
	public UserTeamData generateUserTeamData(String orgId, String toolId, String projectId, String qId) {
		...
		UserTeamData userTeamData = userTeamDataService.generateUserTeamData("SD User Team",
				CommonConstants.SD_USERTEAM_DASHBOARD_ID, orgId, toolId, projectId, qId);
	...
	/**
	 * This method generates the JSON file required for generating the UserTeam
	 * Chart for SD for all the questions.
	 * 
	 * @param orgId: The organization Id.
	 * @param toolId: The tool Id.
	 * @param projectId: The project Id.
	...
	public ArrayList<UserTeamData> generateUserTeamDataAllQuestions(String orgId, String toolId, String projectId) {
		...
		ArrayList<UserTeamData> userTeamData = userTeamDataService.generateUserTeamDataAllQuestions("SD User Team",
				CommonConstants.SD_USERTEAM_DASHBOARD_ID, orgId, toolId, projectId);
